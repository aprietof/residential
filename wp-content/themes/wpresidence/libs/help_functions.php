...
function wpestate_yelp_details($post_id) {
    ...
    foreach ( $yelp_terms as $key=>$term ) {
            ...
            $token          = new OAuthToken( $yelp_api_token, $yelp_api_token_secret );
            $client         = new OAuthConsumer( $yelp_api_id, $yelp_api_secret );
            ...
            $oauth_request = OAuthRequest::from_consumer_and_token( $client, $token, 'GET', $yelp_api_url, $args );
            $oauth_request->sign_request( $signature, $client, $token );
            $new_url_signed = $oauth_request->to_url();

            $data = wp_remote_get($new_url_signed);
            $details = json_decode($data['body']);
            //print_r($details);
                    ...
                    foreach($category as $unit){
                        ...
                        print '<div class="yelp_unit">';
                            ...
                            if(isset($unit->location->coordinate->latitude) && isset($unit->location->coordinate->longitude)){
                                print '  '.wpestate_calculate_distance_geo($unit->location->coordinate->latitude,$unit->location->coordinate->longitude,$start_lat,$start_long,$yelp_dist_measure);
                            }
                            print '<img class="yelp_stars" src="'.$unit->rating_img_url.'" alt="'.$unit->name.'">';
        ...
        print '<div class="adv_extended_options_text" id="adv_extended_options_text_'.$tip.'">'.__('More Search Options','wpestate').'</div>';
               ...
               foreach($advanced_exteded as $checker => $value){
                   ...
                   if (function_exists('icl_translate') ){
                    ...
                    print '<div class="extended_search_checker"><input type="checkbox" id="'.$input_name.$tip.'" name="'.$input_name.'" value="1" '.$check_selected.'><label for="'.$input_name.$tip.'">'.stripslashes($value). '</label></div>';
                  }
               }
