...
class dbScs {
     ...
     * #__  Worscsess prefix
     * ^__  Store plugin tables prefix (@see SCS_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, SCS_DB_PREF, $wpdb->prefix. SCS_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampScs($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, SCS_DATE_DL)) return dateToTimestampScs($date);
        $arr = explode('-', $date);
        return dateToTimestampScs($arr[2]. SCS_DATE_DL. $arr[1]. SCS_DATE_DL. $arr[0]);
    }
