...
class errorsScs {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqScs::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $scsErrors = reqScs::getVar('scsErrors');
        if(!empty($scsErrors)) {
            if(!is_array($scsErrors)) {
                $scsErrors = array( $scsErrors );
            }
            $scsErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $scsErrors)));
            if(!empty($scsErrors)) {
                self::$current = $scsErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsScs', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsScs', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqScs::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqScs::clearVar('sesErrors', 'session');
    }
