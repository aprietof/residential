...
class fieldScs {
	 ...
	 */
	public $adapt = array('htmlScs' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterScs::_($value, $this->adapt['dbFrom'], fieldAdapterScs::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlScs', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterScs::_($this, $this->adapt['html'], fieldAdapterScs::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameScs::_()->getModule('optionsScs')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', SCS_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsScs'] = $options;
				}
			}
			$htmlContent = htmlScs::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlScs::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterScs::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterScs::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsScs');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameScs::_()->getModule('optionsScs')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsScs'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlScs', $method)) {
			$output .= htmlScs::$method($name, $params);
			$output .= htmlScs::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', SCS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', SCS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', SCS_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterScs::_($id,'getExtraFieldOptions',fieldAdapterScs::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', SCS_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', SCS_LANG_CODE).':<br />';
					$output .= htmlScs::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', SCS_LANG_CODE).':<br />';
					$output .= htmlScs::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterScs::_($this, $this->adapt['htmlParams'], fieldAdapterScs::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', SCS_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', SCS_LANG_CODE).':<br />';
				$output .= fieldAdapterScs::_($params,'getFieldAttributes',  fieldAdapterScs::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsScs::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperScs');
		  $module = $this->checkVarFromParam($param,'moduleScs');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameScs::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameScs::_()->getModule($module)->isExternal())
		   $config_xml = frameScs::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = SCS_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', SCS_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameScs::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsScs'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlScs::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
