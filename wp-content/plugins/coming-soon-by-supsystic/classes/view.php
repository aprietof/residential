<?php
abstract class viewScs extends baseObjectScs {
    /* 
     * @deprecated
     */
    protected $_tpl = SCS_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$code = $this->_code;
		$parentModule = frameScs::_()->getModule( $this->_code );
		$plTemplate = frameScs::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !frameScs::_()->getModule($plTemplate))
			$plTemplate = '';
	...
	public function getModel($code = '') {
		return frameScs::_()->getModule( $this->_code )->getController()->getModel($code);
	}
