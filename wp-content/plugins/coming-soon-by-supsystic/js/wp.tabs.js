	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('scsWpTabs')) {
					$this.addClass('scsWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.scsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.scsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.scsTabContent');
					allTabsContent.hide();
