 ...
 */
if(!function_exists('dateToTimestampScs')) {
    function dateToTimestampScs($date) {
        if(empty($a))
            return false;
        $a = explode(SCS_DATE_DL, $date);
...
if(!function_exists('importScs')) {
    function importScs($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassScs')) {
    function importClassScs($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), SCS_CODE) !== false) {
					$classFile = preg_replace('/'. SCS_CODE. '/i', '', $classFile);
				}
                $path = SCS_CLASSES_DIR. $classFile. '.php';
			}
            return importScs($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, SCS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], SCS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameScs')) {
    function toeGetClassNameScs($class) {
        $className = '';
		if(class_exists($class. strFirstUp(SCS_CODE))) 
			$className = $class. strFirstUp(SCS_CODE);
		else if(class_exists(SCS_CLASS_PREFIX. $class)) 
            $className = SCS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjScs')) {
    function toeCreateObjScs($class, $params) {
        $className = toeGetClassNameScs($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectScs')) {
    function redirectScs($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsScs')) {
	function prepareParamsScs(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsScs::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeScs($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeScs')) {
	function prepareFieldCodeScs($string) {   
...
/**
 * Explode string by nultiple delimiters from array
 * @param array $delimiters List of delimiters
 * @param string $string String to explode
 * @return array Result of explode
 */
if(!function_exists('explodeByArrSup')) {
    function explodeByArrSup($delimiters, $string) {
