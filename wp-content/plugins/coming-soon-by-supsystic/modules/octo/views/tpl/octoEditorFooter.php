<?php
	...
	//do_action( 'wp_footer');
	// remove Hooks, to resolve conflict with "Optimizer Theme", for Edit Template page
	remove_action('customize_controls_print_footer_scripts', 'optimizer_customizer_editor', '1');
	remove_action('customize_controls_print_footer_scripts', 'optimizer_customizer_stuff', '1');
...
<div class="modal fade" id="scsSubSettingsWnd" tabindex="-1" role="dialog" aria-labelledby="scsSubSettingsWndLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body scsElMenuSubPanel scsSettingFieldsShell">
				<div id="scsSubSettingsWndTabs">
					<h3 class="nav-tab-wrapper" style="margin-bottom: 0px; margin-top: 12px;">
						...
						<a class="nav-tab scsTabHide scsTabHide_aweber scsTabHide_mailchimp scsTabHide_wordpress" href="#scsSubSetWndConfirmTab">
					...
					<div id="scsSubSetWndMainTab" class="scsTabContent">
						<label class="scsElMenuSubPanelRow">
							...
							<?php echo htmlScs::selectbox('sub_dest', array(
								'options' =>
									frameScs::_()->getModule('subscribe')->getDestListForSelect(),
									'attrs' => ' id="scs-mails-type-sel" ',
							))?>
						</label>
						<label class="scs-displ-none" id="scs-newslet-plug-available"><?= frameScs::_()->getModule('subscribe')->isNewlettersClassAvailable();?></label>
						<label class="scs-displ-none" id="scs-err-msg-NLBS-not-inst" ><?= __("To use this subscribe engine - you must have <a target='_blank' href='" . admin_url('plugin-install.php?tab=search&s=Newsletter+by+Supsystic') . "'>Newsletter by Supsystic plugin</a> installed on your site", SCS_LANG_CODE);?></label>
						<label class="scs-displ-none" id="scs-err-msg-NSBL-no-sel-l"><?= __('Please, Select the value!') ?></label>
						<?php if(frameScs::_()->getModule('subscribe')->isNewlettersClassAvailable()) :
							$newsLetterArr = frameScs::_()->getModule('subscribe')->getNewsletterBySupsysticList();
						?>
							<label class="scsElMenuSubPanelRow scsSubDestRow scsSubDestRow_newsletters">
							<span class="mce-input-name-txt"><?= __('Supsystic Subscription Lists', SCS_LANG_CODE)?></span>
							<?php if(count($newsLetterArr) > 0):
									echo htmlScs::selectbox('sub_newsletters_list', array(
										'options' => $newsLetterArr,
										'attrs' => ' id="scs-subscr-type-sel" ',
									));
								else :
									echo "<label style='font-weight:normal'>" . __("You have no Subscribe lists in Newsletter plugin. Please create new list at first, then select it here.", SCS_LANG_CODE) . "</label>";
								endif;
							?>
							</label>
						<?php endif; ?>
						<label id="scs-subscr-err-msg" class="scsElMenuSubPanelRow scs-displ-none scs-text-danger"></label>
						...
						<label class="scsElMenuSubPanelRow scsSubDestRow scsSubDestRow_aweber scsSubDestRow_mailchimp scsSubDestRow_wordpress">
					...
					<div id="scsSubSetWndNewSubTab" class="scsTabContent">
						...
						<label class="scsElMenuSubPanelRow scsSubDestRow scsSubDestRow_newsletters">
							<span class="mce-input-name-txt"><?php _e('new Subscriber email subject', SCS_LANG_CODE)?></span>
							<?php echo htmlScs::text('sub_txt_subscriber_mail_subject_newsletter', array(
								'attrs' => 'data-default="'. esc_html(__('Subscribe to [sitename]', SCS_LANG_CODE)). '"',
							))?>
						</label>
						<label class="scsElMenuSubPanelRow">
						...
						<label class="scsElMenuSubPanelRow scsSubDestRow scsSubDestRow_newsletters">
							<span class="mce-input-name-txt"><?php _e('new Subscriber email text', SCS_LANG_CODE)?></span>
							<?php echo htmlScs::textarea('sub_txt_subscriber_mail_message_for_newslet', array(
								'attrs' => 'data-default="' . esc_html(__('Hello!<br />Congratulations! You have successfully subscribed to the site <a href="[siteurl]">[sitename]</a>.', SCS_LANG_CODE)) . '"',
							))?>
						</label>
					</div>
				</div>
			...
			<div class="modal-footer">
				<button id="scs-subscr-f-save" type="button" class="button-primary scsSubSettingsSaveBtn"><?php _e('Save', SCS_LANG_CODE)?></button>
			</div>
		</div>
	</div>
</div>
